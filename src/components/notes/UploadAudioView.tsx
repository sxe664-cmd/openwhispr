...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  useEffect(() => {
    // Uploads and URL downloads use the local private space and its folders.
    let cancelled = false;
  ...
  const getBatchSizeErrorKey = (_sizeBytes: number): string | null => null;
  ...
  const cancelTranscription = () => {
    // The run-id bump discards any late result from providers that can't be
    // aborted; local transcription has no cloud request to cancel.
