...
def _build_calendar_block(config: BusinessConfig) -> str:
    ...
    return (
        ...
        "     notes, caller_email, caller_dob, new_patient, sms_consent_opted_in,\n"
        "     details_confirmed) — call this AFTER the caller confirms the\n"
        ...
        "  - Before booking, collect the caller's date of birth in YYYY-MM-DD\n"
        "    form. Never guess or infer it. If the caller explicitly says they\n"
        "    are new, pass new_patient=true; otherwise leave it false so an\n"
        "    ambiguous or unmatched identity fails closed.\n"
        "  - Do not put date of birth, clinical transcript, or audio in notes;\n"
        "    those never belong in Google Calendar.\n"
